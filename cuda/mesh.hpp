...
public:
  ...
  pair<Matrix, vector<double> >& form_sle_stat(const double T_z) const;  // Сама слау для стационарного уравнения
  pair<Matrix, vector<double> >& form_sle_nonstat(const double T_z, const double sigm, const double dt) const;  // Сама слау для нестационарного уравнения
